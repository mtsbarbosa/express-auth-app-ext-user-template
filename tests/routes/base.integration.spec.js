  ...
  describe('GET users', () => {
      ...
      try{
        var users =
            await chai.request(server)
                      .get('/express-auth-app-ext-user-template/users')
    ...
    /*it('should bring status related to header application in case of being root', async() => {
      ...
      try{
        var status =
            await chai.request(server)
                      .get('/express-auth-app-ext-user-template/status/'+basic_fixtures.not_root_status.object._id)
                      .set('x-access-token', auth.body.token)
    ...
    it('should not bring status related to header application in case of not being root', async() => {
      ...
      try{
        var status =
            await chai.request(server)
                      .get('/express-auth-app-ext-user-template/status/'+basic_fixtures.status_active.object._id)
                      .set('x-access-token', auth.body.token)
